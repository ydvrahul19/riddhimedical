...
const allVideos = [
  ...
  { src:'videos/clinic_video10.mp4', thumb:'images/clinic/thumb10.jpg', label:'Recovery Session 10' },
];
...
function buildVideoCard(v, index) {
  ...
  card.style.cssText = `border-radius:16px;overflow:hidden;cursor:pointer;position:relative;aspect-ratio:9/16;background:#0a1527;box-shadow:0 8px 24px rgba(26,46,74,.15);transition:.3s;`;
  card.innerHTML = `
    <img src="${v.thumb}" alt="${v.label}" style="width:100%;height:100%;object-fit:cover;display:block;transition:.35s;">
    <div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(10,21,39,.85) 0%,transparent 50%);opacity:0;transition:.3s;" class="vid-overlay"></div>
    <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:52px;height:52px;background:rgba(255,255,255,.9);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.2rem;transition:.3s;opacity:.85;" class="vid-play">▶</div>
    <div style="position:absolute;bottom:12px;left:12px;right:12px;color:white;font-size:.78rem;font-weight:600;opacity:0;transition:.3s;" class="vid-label">${v.label}</div>
  `;
  card.addEventListener('mouseenter', () => {
    card.querySelector('img').style.transform = 'scale(1.06)';
    card.querySelector('.vid-overlay').style.opacity = '1';
    card.querySelector('.vid-play').style.opacity = '1';
    card.querySelector('.vid-play').style.transform = 'translate(-50%,-50%) scale(1.1)';
    card.querySelector('.vid-label').style.opacity = '1';
  });
  card.addEventListener('mouseleave', () => {
    card.querySelector('img').style.transform = 'scale(1)';
    card.querySelector('.vid-overlay').style.opacity = '0';
    card.querySelector('.vid-play').style.opacity = '.85';
    card.querySelector('.vid-play').style.transform = 'translate(-50%,-50%) scale(1)';
    card.querySelector('.vid-label').style.opacity = '0';
  });
  card.addEventListener('click', () => openVideoModal(v.src, v.label));
  return card;
}
...
function loadMoreVideos() {
  ...
  if (!grid) return;
  const batch = allVideos.slice(videosShown, videosShown + 3);
  batch.forEach(v => {
    const card = buildVideoCard(v, videosShown);
    grid.appendChild(card);
    Motion.scaleIn(card, { duration: 400, delay: 80 * (videosShown % 3) });
    videosShown++;
  });
  if (videosShown >= allVideos.length && btn) btn.style.display = 'none';
}

function openVideoModal(src, label) {
  const modal = document.getElementById('videoModal');
  const vid = document.getElementById('modalVideo');
  const lbl = document.getElementById('videoModalLabel');
  if (!modal || !vid) return;
  vid.src = src;
...
// Init first batch when page loads
